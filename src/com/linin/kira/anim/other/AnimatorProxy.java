...
//    public static final boolean NEEDS_PROXY = Integer.valueOf(Build.VERSION.SDK).intValue() < Build.VERSION_CODES.HONEYCOMB;
    ...
    private void transformMatrix(Matrix m, View view) {
        final float w = view.getWidth();//view的宽
        final float h = view.getHeight();//view的高
        final boolean hasPivot = mHasPivot;//是否设置了中心点坐标
        final float pX = hasPivot ? mPivotX : w / 2f;//默认是中心点位置的x坐标
        final float pY = hasPivot ? mPivotY : h / 2f;//默认是中心点位置的y坐标

        final float rX = mRotationX;//参数
